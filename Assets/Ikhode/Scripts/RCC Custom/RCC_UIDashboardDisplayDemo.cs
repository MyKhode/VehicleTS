...
/// </summary>
[AddComponentMenu("BoneCracker Games/Realistic Car Controller/UI/RCC UI Dashboard Display Demo")]
[RequireComponent(typeof(RCC_DashboardInputsDemo))]
public class RCC_UIDashboardDisplayDemo : MonoBehaviour {

    //  Inputs of the dashboard elements.
    private RCC_DashboardInputsDemo inputs;
    private RCC_DashboardInputsDemo Inputs {

        get {

            if (inputs == null)
                inputs = GetComponent<RCC_DashboardInputsDemo>();
    ...
    private void LateUpdate() {
        ...
        if (KMHLabel) {
            ...
            if (RCC_Settings.Instance.units == RCC_Settings.Units.KMH)
                speedText = Inputs.KMH.ToString("0");
            else
                speedText = (Inputs.KMH * 0.62f).ToString("0");

            if (speedText.Length == 1)
                KMHLabel.text = "00" + speedText;
            else if (speedText.Length == 2)
                KMHLabel.text = "0" + speedText;
            else
                KMHLabel.text = speedText;
        }
